...
namespace ToolBox.Pools
{
    internal sealed class Pool
    {
        ...
        public void Populate(Transform transform, int count)
        {
            for (var i = 0; i < count; i++)
            {
                var instance = CreateInstance(transform);
                instance.gameObject.SetActive(false);
                _instances.Push(instance);
            }
        }
        ...
        private Poolable CreateInstance(Transform transform)
        {
            var instance = Object.Instantiate(_prototype, transform);
            var instanceGameObject = instance.gameObject;
